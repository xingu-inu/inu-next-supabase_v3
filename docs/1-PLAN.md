# INU - Plan Document

> "나를 알아가는 가장 쉬운 방법"
>
> For detailed feature specs, see the [plan/](plan/) folder
> For screen designs (wireframes), see [2-WIREFRAMES.md](2-WIREFRAMES.md)
> For individual wireframes, see the [wireframes/](wireframes/) folder

---

## Document Index

| Section | Title | File | Description |
| ---- | ---- | ---- | ---- |
| 1 | Service Overview | [P-OVERVIEW.md](plan/P-OVERVIEW.md) | Basic info, core philosophy, brand metaphor, design principles |
| 2 | Problem Definition & Target | [P-TARGET.md](plan/P-TARGET.md) | Problem to solve, target users, personas |
| 3 | Core Features | [P-FEATURES.md](plan/P-FEATURES.md) | Goal creation, card view, tree view, coaching, AI, retrospective, etc. (3.1–3.10) |
| 4 | User Flows | [P-FLOWS.md](plan/P-FLOWS.md) | Onboarding, Goal CRUD, daily use, section switching (4.1–4.7) |
| 5 | Information Architecture | [P-IA.md](plan/P-IA.md) | Screen list, sitemap, navigation, URL structure |
| 6 | Category System | [P-CATEGORY.md](plan/P-CATEGORY.md) | Category list, importance, WHY options, Phase/Action templates |
| 7 | Tech Stack | [P-TECH.md](plan/P-TECH.md) | Frontend, Backend, External Services |
| 8 | Data Structure | [P-DATA.md](plan/P-DATA.md) | ER diagram, table list, Phase state transitions |
| 9 | MVP Scope | [P-MVP.md](plan/P-MVP.md) | Priority definition, MVP inclusions/exclusions |

---

## Section Details

### Service Foundations

- [P-OVERVIEW — Service Overview](plan/P-OVERVIEW.md)
- [P-TARGET — Problem Definition & Target](plan/P-TARGET.md)

### Core Features (§3)

- [P-FEATURES — Core Features](plan/P-FEATURES.md)
  - 3.1 구체화하기 (Goal Creation Flow)
  - 3.2 Card View (Default UI)
  - 3.3 Tree View (Goal Structure Visualization)
  - 3.4 구체화하기 (Expanding Existing Goals)
  - 3.5 Adaptive Roadmap & Coaching System
  - 3.6 AI Recommendations (Gemini 3)
  - 3.7 Change History Tracking
  - 3.8 Cross-Goal Mapping
  - 3.9 Retrospective System
  - 3.10 Data Utilization: Insights

### User Flows & Design

- [P-FLOWS — User Flows](plan/P-FLOWS.md)
- [P-IA — Information Architecture](plan/P-IA.md)

### Data & Technology

- [P-CATEGORY — Category System](plan/P-CATEGORY.md)
- [P-TECH — Tech Stack](plan/P-TECH.md)
- [P-DATA — Data Structure](plan/P-DATA.md)

### Scope Definition

- [P-MVP — MVP Scope](plan/P-MVP.md)

### Screen Design

> For screen designs (wireframes), see [2-WIREFRAMES.md](2-WIREFRAMES.md)

---

*Document version: v7.3*
*Last updated: January 31, 2026*
